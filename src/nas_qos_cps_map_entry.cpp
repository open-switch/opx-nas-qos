                                            ...
                                            nas_qos_map_type_t map_type)
{
    ...
    if ((rc = qos_map_key_check(obj, map_type, &is_map_entry)) != cps_api_ret_code_OK) {
        EV_LOGGING(QOS, DEBUG, "NAS-QOS", "Key error");
        return rc;
    }
    ...
    if (entry_attr == NULL && is_map_entry) {
        ...
        EV_LOGGING(QOS, DEBUG, "NAS-QOS", "Key value must be specified for inner list\n");
                                        ...
                                        )
{
    ...
    obj_id = qos_map_map_name_obj_id(map_type);
    cps_api_object_attr_add(ret_obj, obj_id,
            map->get_name(), MAP_NAME_LEN);
                                ...
                                nas_qos_map_type_t map_type)
{
    ...
    nas_qos_map_entry_key_t key;
    cps_api_return_code_t rc = cps_api_ret_code_OK;
    bool map_set= false; //setting map attr vs. map-entry att
    ...
    if (map_set) {
        // map key ok, setting map attributes.
        // currently only map-name can be set
        // Since this attr is not passed to SAI, no rollback is necessary.

        nas_attr_id_t obj_id = qos_map_map_name_obj_id(map_type);
    ...
    if (it == entries->end()) {
        EV_LOGGING(QOS, DEBUG, "NAS-QOS", "Map entry does not exist in NAS\n");
    ...
    try {
        ...
        if (entry.is_created_in_ndi() == false)
            EV_LOGGING(QOS, DEBUG, "NAS-QOS", "map is yet not created in ndi \n");
