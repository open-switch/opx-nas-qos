                                ...
                                cps_api_object_list_t sav_obj)
{
    ...
    if (policer.npu_list().size() == 0) {
        EV_LOGGING(QOS, DEBUG, "NAS-QOS", "New Policer has no npu id on file\n");
    }
    else {
        for (auto npu_id: policer.npu_list()) {
            EV_LOGGING(QOS, DEBUG, "NAS-QOS", "New Policer npu id: %d on file\n", npu_id);
        }
    }


    if (policer_p->npu_list().size() == 0) {
        EV_LOGGING(QOS, DEBUG, "NAS-QOS", "Existing Policer has no npu id on file\n");
    }
    else {
        for (auto npu_id: policer_p->npu_list()) {
            EV_LOGGING(QOS, DEBUG, "NAS-QOS", "Existing Policer npu id: %d on file\n", npu_id);
        }
    }
